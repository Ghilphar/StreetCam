...
while True:
    ...
    if ret:
        ...
        # Person detection using Haar Cascade Upper Body (fine-tuned settings)
        bodies = person_cascade.detectMultiScale(
            gray,
            scaleFactor=1.1,    # Fine-tune the scaling factor
            minNeighbors=3,      # Adjust neighbor count for sensitivity
            minSize=(50, 50),    # Minimum size of detected person
            maxSize=(800, 800)   # Optional: Max size of detected person
        )

        # Draw green rectangles for each detected person
        ...
        # Draw the ROI as a red rectangle on the display frame (motion detection area)
